...
def read_line_from_file(file_name, line_number):
    with open(file_name, 'r') as file:
        for line_no, line in enumerate(file):
            ...
            file.close()
            raise ValueError(
                'line %s does not exist in file %s' % (line_number, file_name)
            )

line = read_line_from_file('../input_parameters.in', 1)

# Function that sets parameters according to atom_type chosen per last
# simulation
...
def plot_temperature(data_file, atom, epsilon, label, color):
    ...
    axis[0].set_title("Adimensional simulation")
